...
class ControllerApp(App): 
    def build(self):
        ...
        return Controller(info="1",label_wid="r")
    ...
    def on_start(self):
        ...
        def onMessage(client, userdata, msg):
            ...
            if msg.topic == "kivy/button":
                userdata['self'].root.info = msg.payload

            if msg.topic == "kivy/text":
                userdata['self'].root.label_wid = msg.payload

        parameters = {'self': self}
        mqttc = mqtt.Client(client_id="kivy-client", clean_session=True, userdata = parameters)
        mqttc.on_connect = onConnect 
        mqttc.on_message = onMessage
        mqttc.connect("test.mosquitto.org", 1883, keepalive=60, bind_address="")
